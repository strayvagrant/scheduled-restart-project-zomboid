...
async def schedule_restart():
    ...
    while True:
        ...
        # Find the next closest restart time
        ...
        for hour, minute in RESTART_TIMES:
            ...
            if restart_time < now:
                restart_time += timedelta(days=1)  # Move to the next day if it's already past

            wait_time = (restart_time - now).total_seconds()

            if wait_time < min_wait_time:
                min_wait_time = wait_time
                next_restart_time = restart_time

        print(f"Next restart scheduled at: {next_restart_time} (in {min_wait_time} seconds)")
        await asyncio.sleep(min_wait_time - 1800)  # Wait until 30 min before restart

        # Countdown messages with "save" after each
        for msg, delay in [("30 minutes", 900), ("15 minutes", 600), ("5 minutes", 240), ("1 minute", 60)]:
            await send_message(f"Server will do a scheduled restart in {msg}!")
            await send_ingame_message(f"Server will do a scheduled restart in {msg}!", save=True)  # Save after each message
